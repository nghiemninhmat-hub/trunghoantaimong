...
## Nguyên nhân gốc rễ
- Cập nhật auth.users.encrypted_password trực tiếp bằng crypt() KHÔNG hoạt động
  với GoTrue auth server — mật khẩu không dùng để đăng nhập được
- Cách duy nhất đáng tin cậy: gọi Auth Admin API qua HTTP

## Giải pháp
- admin_update_user_password (gọi từ DB) giờ gọi edge function qua pg_net
- Edge function gọi Auth Admin API (cách chính thống)
- Edge function hỗ trợ 2 chế độ:
  1. Ban quản lý web: session token (Authorization: Bearer)
  2. Quản trị web: X-Admin-Token = service_role_key (từ DB qua pg_net)
- admin_save_password_change: đồng bộ profiles + history + revoke sessions
*/
...
BEGIN
  ...
  IF v_service_key IS NULL THEN
    RAISE EXCEPTION 'Chưa cấu hình service role key. Vui lòng chạy: SELECT vault.create_secret(''edge-function-service-key'', ''<service_role_key>'', ''Service key for edge functions'');';
  END IF;
  ...
  IF v_status IS NULL THEN
    RAISE EXCEPTION 'Edge function không phản hồi sau 5 giây.';
  END IF;
