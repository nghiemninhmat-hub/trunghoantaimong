/*
# Fix ambiguous column reference in spin_wheel()

## Problem
The function `spin_wheel()` returns `TABLE(reward_key text, reward_label text, ...)`.
Inside the function body, the `INSERT INTO wheel_spin_log (reward_key, ...)`
statement references column `reward_key`. PostgreSQL resolves this as
ambiguous — it could be the function's output column OR the table's column.
When called via PostgREST (as authenticated role), this causes:
  ERROR: column reference "reward_key" is ambiguous (SQLSTATE 42702)
This is why every 26th spin (and possibly other spins) fail with
"Không thể quay lúc này. Vui lòng thử lại sau."

## Fix
1. DROP the existing function (return type changes, so DROP+CREATE is required).
2. Recreate with `p_` prefixed return column names to avoid collision.
3. Keep all internal variable names with `v_` prefix.
4. The INSERT statements now unambiguously reference table columns.

## Frontend Impact
The RPC response keys change from `reward_key` → `p_reward_key`, etc.
The frontend must be updated to read the new keys.
...
BEGIN
  ...
  UPDATE public.profiles
    ...
    RETURNING wheel_spins, wheel_total_spins, wheel_special_claimed, oc_name
    INTO v_spins, v_total_spins, v_special_claimed, v_oc_name;
  ...
  -- Guaranteed special at exactly 26 total spins, once per account
