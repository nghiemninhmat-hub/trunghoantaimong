/*
# Fix: restore p_ prefixed return columns + exact 26th spin special

## Problem
The previous migration (fix_wheel_special_exact_26th_spin) dropped and
recreated spin_wheel() with non-prefixed return column names
(reward_key, reward_label, ...). But the frontend TypeScript interface
WheelSpinResult expects p_-prefixed names (p_reward_key, etc.), and the
ambiguous-column fix from migration 20260906044832 specifically used p_
prefixes to avoid PostgreSQL "column reference is ambiguous" errors.

This caused every spin result to have undefined fields in the frontend.

## Fix
1. DROP the function (return type changes require DROP+CREATE).
2. Recreate with p_ prefixed return columns, matching the frontend.
3. Keep the exact-26th-spin logic: v_total_spins = v_threshold (not >=).
...
BEGIN
  ...
  -- Guaranteed special ONLY on exactly the 26th spin, once per account
