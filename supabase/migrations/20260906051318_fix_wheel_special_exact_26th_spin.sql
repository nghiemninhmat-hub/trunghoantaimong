/*
# Fix: special reward only on exactly the 26th spin

The previous logic used `v_total_spins >= 26`, meaning any spin at or
after 26 could trigger the special reward if not yet claimed. The
requirement is that the special reward may ONLY be awarded on exactly
the 26th spin — not on spin 27, 28, etc.

Change `>=` to `=` so the special is only possible on the exact 26th
spin. If special stock was empty on spin 26, the opportunity is lost.
*/
