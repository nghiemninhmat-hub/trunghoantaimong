/*
# Simplify wheel special reward to fixed 26-spin threshold

## Problem
The `wheel_special_threshold` column on `profiles` allows per-account
custom thresholds. The user wants a single universal rule: every account
that reaches exactly 26 total spins gets a guaranteed special reward,
once per account — no per-account configuration needed.

## Changes
1. Rewrite `spin_wheel()` to use a hardcoded constant `26` instead of
   reading `wheel_special_threshold` from the profile row.
2. Reset the one account with a custom threshold (46) back to 26 so
   they also get the special reward at 26 like everyone else.
3. The `wheel_special_threshold` column is kept (not dropped — data
   safety) but no longer read by the function.
4. `wheel_special_claimed` still enforces one-per-account.
...
-- Reset the one account with custom threshold back to 26
UPDATE public.profiles SET wheel_special_threshold = 26 WHERE wheel_special_threshold != 26;
