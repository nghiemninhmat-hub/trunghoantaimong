/*
# Log special wheel rewards in transactions

## Problem
When a player wins a special reward from the wheel (26th spin), the reward
is logged in `wheel_spin_log` and stock is decremented, but NO entry is
created in `transactions`. So the special reward doesn't appear in the
player's "Lịch Sử Giao Dịch" (transaction history) like other rewards do.

## Fix
Update `spin_wheel()` to also INSERT into `transactions` when a special
reward is won. The transaction row uses:
  - amount = 0
  - currency_type = NULL (allowed by the CHECK constraint)
  - reason = 'Quà Đặc Biệt: <reward_label>'

This makes special rewards visible in the transaction history alongside
currency rewards and item rewards.
...
BEGIN
  ...
  -- Log special reward in transactions so it appears in history
  IF v_group = 'SPECIAL' THEN
    INSERT INTO public.transactions (user_id, amount, currency_type, reason)
      VALUES (v_uid, 0, NULL, 'Quà Đặc Biệt: ' || v_reward_label);
  END IF;
