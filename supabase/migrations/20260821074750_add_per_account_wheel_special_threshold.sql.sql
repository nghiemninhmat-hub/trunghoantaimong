/*
# Per-account wheel special reward threshold

1. Overview
- The lucky wheel currently hardcodes the special-reward threshold at 26 total spins for every account.
- This migration adds a per-account override column `wheel_special_threshold` (default 26) so admins can require more (or fewer) spins for specific accounts before the guaranteed special reward triggers.
- The spin_wheel() function is updated to compare wheel_total_spins against the account's own threshold instead of the hardcoded 26.

2. New column on `profiles`
- `wheel_special_threshold` int NOT NULL DEFAULT 26 — the total-spin count at which the guaranteed special reward becomes eligible for this account.

3. Changes to `spin_wheel()`
- Replaced the hardcoded `v_total_spins >= 26` check with `v_total_spins >= wheel_special_threshold` (read from the profiles row during the atomic update).
- All other logic unchanged.

4. Security
- `wheel_special_threshold` is added to the column-level REVOKE (not client-writable), same as wheel_spins / wheel_total_spins / wheel_special_claimed.
- No policy changes.
*/

ALTER TABLE public.profiles
  ADD COLUMN IF NOT EXISTS wheel_special_threshold int NOT NULL DEFAULT 26;
...
BEGIN
  ...
  UPDATE public.profiles
    ...
    RETURNING wheel_spins, wheel_total_spins, wheel_special_claimed, wheel_special_threshold
      INTO v_spins, v_total_spins, v_special_claimed, v_threshold;
