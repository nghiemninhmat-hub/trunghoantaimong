-- Fix: spin_wheel() now creates transaction records for ALL reward types,
-- not just currency rewards. Previously SHOP items and SPECIAL prizes
-- were not logged in transactions, so they didn't appear in the player's
...
BEGIN
  ...
  -- Log SHOP item rewards in transactions
  IF v_group = 'SHOP' AND v_stock_id IS NOT NULL THEN
    INSERT INTO public.transactions (user_id, amount, currency_type, reason)
    VALUES (v_uid, 0, NULL, 'Vòng quay may mắn — Vật phẩm: ' || v_reward_label);
  END IF;

  -- Log SPECIAL rewards in transactions
...
-- Grant execute to authenticated (preserve existing grants)
