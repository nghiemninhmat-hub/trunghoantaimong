  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      ...
      {loading ? <div className="flex justify-center py-24"><div className="h-10 w-10 animate-spin rounded-full border-2 border-[#eeb337]/25 border-t-[#eeb337]" /></div> : <>
        ...
        {!selected && <div className="rounded-2xl border border-dashed border-[#eeb337]/20 bg-[#130807]/50 px-5 py-5 text-center"><Sparkles className="mx-auto h-5 w-5 text-[#eeb337]/70" /><p className="mt-2 text-sm text-[#c9b493]">Chọn một vị trí để mở hồ sơ vinh danh</p></div>}

        <section className="grid gap-4 sm:grid-cols-2"><div className="rounded-2xl border border-[#eeb337]/15 bg-[#130807]/70 p-5"><div className="flex items-center gap-2"><ScrollText className="h-4 w-4 text-[#eeb337]" /><h2 className="font-serif text-lg font-bold text-[#fff1cf]">Quy tắc đề danh</h2></div><ul className="mt-4 space-y-2 text-sm leading-6 text-[#c9b493]"><li>Kim Bảng chỉ ghi nhận 06 vị trí cao nhất.</li><li>Thứ hạng dựa trên thành tích Dị Sự và tài phú.</li><li>Thông tin được Quản Trị Viên cập nhật theo từng kỳ.</li></ul></div><div className="rounded-2xl border border-[#eeb337]/15 bg-[#130807]/70 p-5"><div className="flex items-center gap-2"><Trophy className="h-4 w-4 text-[#eeb337]" /><h2 className="font-serif text-lg font-bold text-[#fff1cf]">Lời lưu danh</h2></div><p className="mt-4 font-script text-base italic leading-8 text-[#d7a96d]">Kim bảng lưu danh, thiên hạ chứng kiến.<br />Một bước đề danh, vạn người biết tiếng.</p><div className="mt-4 flex items-center gap-2 text-xs text-[#8a6a4a]"><Coins className="h-3.5 w-3.5" /> Không xét xuất thân, chỉ xét những gì đã làm được.</div></div></section>
      </>}
    </div>
  );
}
